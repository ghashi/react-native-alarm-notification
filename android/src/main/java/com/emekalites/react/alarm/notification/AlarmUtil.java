...
public class AlarmUtil {
  ...
  private static int volumeBeforePlayingAlarm;
  ...
  private NotificationManager getNotificationManager() {
    return (NotificationManager) mContext.getSystemService(
  ...
  private void playAlarmSound(String name, boolean shouldLoop, double volume) {
    ...
    MediaPlayer mediaPlayer = audioInterface.getPlayerForFilename(
      name,
      number,
      shouldLoop
    );
  ...
  boolean checkAlarm(ArrayList<AlarmModel> alarms, AlarmModel alarm) {
    ...
    if (contain) {
      Toast
        .makeText(
          mContext,
          "You have already set this Alarm",
          Toast.LENGTH_SHORT
        )
        .show();
    }
  ...
  void setAlarm(AlarmModel alarm) {
    ...
    if (scheduleType.equals("once")) {
      ...
      long interval =
        this.getInterval(alarm.getInterval(), alarm.getIntervalValue());
  ...
  public void sendNotification(AlarmModel alarm) {
    try {
      ...
      if (title == null || title.equals("")) {
        ...
        title =
          mContext.getPackageManager().getApplicationLabel(appInfo).toString();
      }
      ...
      if (message == null || message.equals("")) {
        Log.d(
          TAG,
          "Cannot send to notification centre because there is no 'message' found"
        );
        return;
      }
      ...
      if (channelID == null || channelID.equals("")) {
        Log.d(
          TAG,
          "Cannot send to notification centre because there is no 'channel' found"
        );
        return;
      }
      ...
      Uri soundUri = Uri.parse(
        "android.resource://" + mContext.getPackageName() + "/" + R.raw.silence
      );
      ...
      long[] vibrationPattern = vibration == 0
      ...
      if (
        ...
        int largeIconResId = res.getIdentifier(
          largeIcon,
          "mipmap",
          packageName
        );
  ...
  void stopAlarmSound() {
    try {
      ...
      Log.e(
        TAG,
        "stopAlarmSound ~ setStreamVolume: " +
        volumeBeforePlayingAlarm +
        " " +
        audioManager.getStreamVolume(AudioManager.STREAM_ALARM)
      );
